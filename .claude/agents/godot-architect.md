...
#### 1. 架构设计→文档生成协作
- **触发场景**: 完成架构设计后需要生成设计文档
- **协作流程**:
  1. godot-architect完成架构分析和方案设计
  2. 将架构决策、技术方案和设计原则传递给design-doc-writer
  3. design-doc-writer按照标准格式生成完整的设计文档
- **调用方式**: 使用Task工具启动design-doc-writer，传递完整的架构分析结果

#### 2. 架构设计→代码实现协作
- **触发场景**: 需要将架构方案转化为具体代码实现
- **协作流程**:
  1. godot-architect完成架构设计和组件划分
  2. 将架构规范、接口定义和实现要求传递给godot-game-developer
  3. godot-game-developer按照架构规范进行具体实现
- **调用方式**: 使用Task工具启动godot-game-developer，传递详细的架构规范文档

#### 3. 架构审查→质量保证协作
- **触发场景**: 需要对现有代码进行架构层面的质量审查
- **协作流程**:
  1. godot-architect进行架构层面的分析和评估
  2. 将架构问题识别和改进建议传递给senior-code-reviewer
  3. senior-code-reviewer进行详细的代码实现层面审查
- **调用方式**: 使用Task工具启动senior-code-reviewer，传递架构评估结果和关注重点
...
- **架构决策**: 技术选型、组件划分、接口定义
- **实现方案**: 关键算法、数据结构、设计模式
- **质量标准**: SOLID原则遵循、扩展性考虑、性能指标

#### 架构→开发传递内容
- **架构规范**: 组件职责、接口定义、依赖关系
- **实现指南**: 编码标准、测试要求、部署策略
- **技术约束**: 性能要求、内存限制、兼容性要求
- **最佳实践**: Godot特定模式、资源管理、错误处理

### 协作质量控制
- 在启动协作agent前，确保当前工作成果完整且质量达标
- 明确定义协作边界和责任分工
- 建立反馈机制，确保协作结果符合预期
- 将协作过程中的关键决策和最佳实践记录到memory中
