...
1. **代码环境分析** (使用 sequential-thinking 工具)
   ...
   - 确定编程语言、框架和技术栈
   - 评估代码规模和复杂度
   - 根据 **$3** 审查深度制定审查策略和检查清单

2. **质量标准建立** (使用 memory 工具)
   - 查询编程语言的最佳实践和编码规范
   - 根据 **$2** 审查类型确定代码质量度量标准和评估指标
   - 收集相关的行业标准和质量要求
   - 为 **$1** 项目建立适合的质量基准
...
3. **审查范围界定** (使用 filesystem 工具)
   - 扫描和识别 **$1** 项目需要审查的代码文件
   - 分析代码依赖关系和影响范围
   - 根据 **$2** 审查类型确定审查的优先级和重点领域
   - 基于 **$3** 审查深度准备审查工具和分析环境
...
2. **根因分析** (使用 memory 工具)
   - 深入分析 **$1** 项目问题的根本原因
   - 查找类似问题的历史案例和解决方案
   - 根据 **$3** 审查深度评估修复方案的可行性和风险
   - 制定预防措施和长期改进策略

3. **改进方案设计** (使用 code-reviewer Agent)
   - 为 **$1** 项目设计具体的代码修改和重构方案
   - 创建重构建议和最佳实践指导
   - 根据 **$3** 审查深度制定代码质量改进的实施计划
   - 提供工具和自动化改进建议
...
1. **审查报告生成** (使用 design-doc-writer Agent)
   - 为 **$1** 项目生成详细的代码审查报告
   - 创建问题清单和修复指导
   - 根据 **$3** 审查深度制作质量度量和趋势分析
   - 基于 **$2** 审查类型编写改进建议和最佳实践

2. **重构指导完善** (使用 sequential-thinking 工具)
   - 优化 **$1** 项目的重构方案和实施步骤
   - 制定重构风险控制和质量保证措施
   - 根据 **$3** 审查深度设计重构验证和测试策略
   - 建立持续改进和监控机制
