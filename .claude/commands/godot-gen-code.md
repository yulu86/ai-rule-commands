...
3. **并行架构设计与任务分析**：
   - **并行架构分析组**（同时启动多个agent进行架构设计）:
     * **主导架构设计**: Task工具启动godot-architect代理进行架构设计
       - 分析编码任务 $ARGUMENTS 的技术需求和架构要求
       - 设计符合SOLID原则的模块化架构方案
       - 提供详细的组件划分和接口定义
       - 输出标准化的架构设计文档
     * **技术可行性分析**: Task工具启动local-developer进行并行技术分析
       - 分析架构方案的Godot引擎实现可行性
       - 识别关键的技术挑战和解决方案
       - 提供初步的技术实现路径
     * **代码质量规划**: Task工具启动senior-code-reviewer制定质量标准
       - 制定代码质量标准和审查规范
       - 识别潜在的质量风险和预防措施
   - **方案整合确认**: sequential-thinking工具整合所有并行分析结果
   - 向用户展示架构方案核心思路，请求确认
   - 如用户不认可，重新进行并行架构设计直到用户满意
   - **设计记录**: 将架构方案记录到memory中供后续开发参考

4. **并行任务分解与责任分配**：
   - **并行任务分解组**（同时进行多维度任务分解）:
     * **结构化分解**: 继续使用godot-architect进行结构化任务分解
       - 将整体架构方案分解为具体可执行的子任务
       - 分析任务间的依赖关系，制定最优执行顺序
       - 识别每个任务的技术复杂度和实现难点
     * **实现复杂度评估**: local-developer评估实现难度和技术风险
       - 评估每个任务的具体实现难度
       - 识别技术难点和解决方案
       - 提供实现时间估算建议
     * **质量风险评估**: senior-code-reviewer进行质量风险评估
       - 识别每个任务可能的质量问题
       - 制定针对性的质量保证措施
       - 设计质量检查点和审查标准
   - **智能责任分配**: 基于并行分析结果进行智能责任划分
...
5. **智能依赖分析与并行执行规划**：
   - **依赖关系分析**: sequential-thinking工具分析所有任务间的复杂依赖关系
   - **并行执行图设计**: 生成支持并行执行的多agent协作执行序列
   - **关键路径识别**: 识别影响整体进度的关键任务路径
   - **并行化机会识别**: 找出可以同时执行的任务组合
   - **质量关卡设置**: 在关键节点设置质量检查点和同步点
...
6. **并行Agent协同开发执行**：
   - **智能代码实现策略**:
     ...
     * **并行实现方法**:
       ...
       - **主要实现组**: 根据依赖关系，并行启动多个local-developer处理独立任务
       - **专业实现组**: 并行启动godot-game-developer处理复杂游戏逻辑
       - **质量监控组**: 并行启动senior-code-reviewer进行实时质量监控

   - **并行代码实现阶段**（基于依赖分析结果最大化并行执行）:
     * **框架生成**: Claude Code生成整体代码框架和架构设计
     * **独立任务并行实现**: 根据依赖分析，对无依赖关系的任务同时启动多个local-developer
       - 每个local-developer严格按照架构设计进行代码实现
       - 遵循 `gdscript编码规范` 和最佳实践
       - 为每个任务生成完整的可执行代码
     * **复杂逻辑并行实现**: 对复杂任务启动godot-game-developer与local-developer协作
       - godot-game-developer处理游戏特定的复杂逻辑和架构
       - local-developer提供基础实现支持
     * **实时质量监控**: senior-code-reviewer在开发过程中提供实时质量建议

   - **并行质量保证阶段**（代码完成后立即启动并行验证）:
     * **并行质量检查组**: 同时启动多个质量检查agent
       - **基础质量检查**: local-developer进行基础的代码质量检查
       - **专业代码审查**: senior-code-reviewer进行深度代码质量审查
         - 验证代码质量、性能和安全性
         - 检查是否符合架构设计和编码规范
         - 消除代码异味和潜在问题
       - **Godot专业审查**: godot-game-developer进行游戏逻辑专业审查
       - **架构一致性检查**: godot-architect验证架构实现的正确性
     * **并行功能测试组**: 同时进行多维度测试
       - 使用godot工具进行功能测试
       - bash命令进行集成测试
       - 性能和稳定性验证
   ...
   - **并行文档化阶段**:
     * **主要文档生成**: Task工具启动design-doc-writer生成任务文档和API文档
     * **技术文档补充**: local-developer编写技术实现细节文档
     * **架构文档**: godot-architect编写架构设计文档
     * 将所有文档保存到 `docs/tasks/` 目录
