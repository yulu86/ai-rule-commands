...
- **并行Git状态分析**（同时进行多维度状态分析）:
  ...
  - **内容分析组**（并行执行）:
    ...
    * 使用 `git log --oneline -10` 分析最近的commit历史模式
  - **文件类型分析组**（并行执行）:
    * 识别和分析配置文件变更（`.json`, `.yaml`, `.toml`, `.env`）
    * 分析代码文件变更（`.js`, `.ts`, `.py`, `.gd`, `.cs` 等）
    * 检查文档文件变更（`.md`, `.txt`, `.rst`）
    * 分析构建文件变更（`Dockerfile`, `Makefile`, 构建脚本）
...
- **并行分析策略**（同时进行多层次分析）:
  - **基础分析组**（并行执行）:
    - local-developer(qwen3-coder:30b)进行快速的代码语义分析
      - 分析变更文件的基本类型和功能
      - 识别简单的代码模式和函数修改
      - 生成基础的commit消息建议
    - 文件路径模式分析（基于路径推断变更类型）
  - **深度分析组**（复杂变更时并行执行）:
    - sequential-thinking工具进行深度逻辑分析
      - 分析架构变更和设计模式修改
      - 识别跨文件的系统性变更
      - 评估变更的影响范围和重要性
    - 影响范围深度分析（评估变更的广度和深度）
...
- **并行预览展示**（同时准备多种展示信息）:
  - **文件展示组**（并行执行）:
    - 展示检测到的所有变更文件（按类型分组）
    - 显示变更统计信息（文件数量、代码行数变化）
    - 生成变更文件的树状结构视图
  - **消息预览组**（并行执行）:
    - 显示生成的 commit 消息预览（多种格式）
    - 提供不同版本的commit消息选择
    - 展示消息质量评分和建议
