...
7. **临时文件清理**：任务执行中生成的临时文件（如中间产物、缓存、调试文件、临时脚本等），在任务结束后**必须**删除；仅当用户明确要求保留或文件为最终交付物时方可保留
8. **需要用户操作的指令用醒目颜色标记**：TUI中凡需用户动手的部分一律用 🟧 橙色或加粗高亮，并加上分割线，与 AI 自动完成的部分视觉区分，降低用户扫读成本。
9. **信息保密**：**禁止**在 git 纳管的文件（代码、文档、脚本、配置等）中包含环境变量、密码、密钥、Token 等具体的值；**必须**使用占位符（如 `${ENV_VAR}`、`<your-secret>`）或从 `.env`/环境变量中读取，避免信息泄露
10. **跨平台兼容**：所有产出的方案、脚本、代码、文档等内容**必须**支持跨平台（macOS、Windows、Linux）；优先使用跨平台工具或语言（如 Node.js、Python、PowerShell 兼容写法等），避免使用平台专属的命令或路径分隔符硬编码
